...
{
    private $conn;
    private $table_name = "usuario"; // Alterei para o nome correto da tabela
    ...
    public function registrar($nome, $email, $senha, $fone, $sexo)
    {
        $query = "INSERT INTO " . $this->table_name . " (nome, email, senha, fone, sexo) VALUES (?, ?, ?, ?, ?)";
        $stmt = $this->conn->prepare($query);
        $hashed_password = password_hash($senha, PASSWORD_BCRYPT); // Hash da senha
        return $stmt->execute([$nome, $email,$hashed_password,$fone, $sexo]);
    }
    ...
    public function atualizar($id, $nome, $sexo, $fone, $email)
    {
        ...
        $stmt = $this->conn->prepare($query);
        return $stmt->execute([$nome, $sexo, $fone, $email, $id]);
    }
